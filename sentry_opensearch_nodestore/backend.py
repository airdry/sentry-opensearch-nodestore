...
class AsyncOpenSearchNodeStorage(NodeStorage):
    ...
    async def bootstrap(self):
        ...
        try:
            ...
            self.logger.info("bootstrap.template.check", extra={"template": self.template_name, "status": "exists"})
        except opensearchpy.exceptions.NotFoundError:
            self.logger.info("bootstrap.template.check", extra={"template": self.template_name, "status": "not found"})
            self.logger.info("bootstrap.template.settings", extra={"shards": self.number_of_shards, "replicas": self.number_of_replicas, "index_pattern": self.index_pattern, "codec": self.index_codec})
            template_body = {
                ...
                "template": {
                    ...
                    "mappings": {
                        ...
                        "dynamic_templates": [],
                        "properties": {
                            "data": {"type": "binary", "doc_values": False, "store": True},
            ...
            await self.os.indices.put_index_template(name=self.template_name, body=template_body, create=True)
            self.logger.info("bootstrap.template.create", extra={"template": self.template_name, "alias": self.alias_name})
    ...
    async def _get_read_index(self, id: str) -> str | None:
        try:
            search_result = await self.os.search(index=self.alias_name, body={"query": {"term": {"_id": id}}})
    ...
    async def delete(self, id: str):
        try:
            await self.os.delete_by_query(index=self.alias_name, body={"query": {"term": {"_id": id}}}, refresh=self.refresh, wait_for_completion=True)
        except (opensearchpy.exceptions.NotFoundError, opensearchpy.exceptions.ConflictError):
            pass
    ...
    async def delete_multi(self, id_list: list[str]):
        if not id_list:
            return
        try:
            await self.os.delete_by_query(index=self.alias_name, body={"query": {"ids": {"values": id_list}}}, refresh=self.refresh, wait_for_completion=True)
        except (opensearchpy.exceptions.NotFoundError, opensearchpy.exceptions.ConflictError):
            pass
    ...
    async def _get_bytes(self, id: str) -> bytes | None:
        ...
        if index is not None:
            try:
                ...
                return None
        self.logger.warning("document.get.warning", extra={"doc_id": id, "error": "index containing doc_id not found"})
        return None

    async def _set_bytes(self, id: str, data: bytes, ttl=None):
        index = self._get_write_index()
        document_body = {'data': self._compress(data), 'timestamp': datetime.now(timezone.utc).isoformat()}
        await self.os.index(id=id, index=index, body=document_body, refresh=self.refresh)

    async def cleanup(self, cutoff: datetime):
        try:
            aliases_response = await self.os.indices.get_alias(name=self.alias_name)
        except opensearchpy.exceptions.NotFoundError:
            self.logger.warning("cleanup.alias.not_found", extra={"alias": self.alias_name})
            return
        for index in aliases_response.keys():
            try:
                index_date_str = '-'.join(index.split('-')[1:4])
                index_ts = datetime.strptime(index_date_str, "%Y-%m-%d").replace(tzinfo=timezone.utc)
            ...
            if index_ts < cutoff:
                try:
                    ...
                    self.logger.info("index.delete.error", extra={"index": index, "error": "not found"})

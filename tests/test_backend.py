...
class TestInitialization:
    ...
    def test_defaults(self, monkeypatch, mock_os_client):
        monkeypatch.delenv("SENTRY_NODESTORE_OPENSEARCH_NUMBER_OF_SHARDS", raising=False)
        monkeypatch.delenv("SENTRY_NODESTORE_OPENSEARCH_NUMBER_OF_REPLICAS", raising=False)
    ...
    def test_env_vars_override(self, monkeypatch, mock_os_client):
        ...
        monkeypatch.setenv("SENTRY_NODESTORE_OPENSEARCH_INDEX_PATTERN", "custom-events-*")
    ...
    def test_invalid_env_vars_fallback(self, monkeypatch, mock_os_client):
        monkeypatch.setenv("SENTRY_NODESTORE_OPENSEARCH_NUMBER_OF_SHARDS", "not-a-number")
...
class TestBootstrap:
    ...
    async def test_creates_template_if_not_found(self, nodestore, mock_os_client):
        ...
        mock_os_client.indices.get_index_template.assert_awaited_once_with(name=nodestore.template_name)
...
class TestCRUDOperations:
    ...
    async def test_get_bytes_found(self, nodestore, mock_os_client):
        # ... (test implementation is unchanged) ...
        mock_os_client.search.return_value = {"hits": {"total": {"value": 1}, "hits": [{"_index": "sentry-2025-08-26"}]}}
        ...
        mock_os_client.search.assert_awaited_once_with(index=nodestore.alias_name, body=ANY)
        mock_os_client.get.assert_awaited_once_with(id=self.NODE_ID, index="sentry-2025-08-26", stored_fields=["data"])
    ...
    async def test_delete(self, nodestore, mock_os_client):
        ...
        mock_os_client.delete_by_query.assert_awaited_once_with(index=nodestore.alias_name, body=expected_query, refresh=ANY, wait_for_completion=ANY)
...
class TestCleanup:
    ...
    async def test_cleanup_deletes_old_indices(self, nodestore, mock_os_client):
        ...
        mock_os_client.indices.get_alias.return_value = {old_index: {"aliases": {nodestore.alias_name: {}}}, new_index: {"aliases": {nodestore.alias_name: {}}}, "sentry-2025-01-01-reindexed": {"aliases": {nodestore.alias_name: {}}}}
        await nodestore.cleanup(cutoff)
        mock_os_client.indices.get_alias.assert_awaited_once_with(name=nodestore.alias_name)
        assert mock_os_client.indices.delete.await_count == 2
        mock_os_client.indices.delete.assert_any_await(index=old_index)
        mock_os_client.indices.delete.assert_any_await(index="sentry-2025-01-01-reindexed")
